...
Cleaned translation artifact for HotpotQA to Indonesian using NLLB-200 1.3B.

Original workflow:
- Translate HotpotQA train and validation data.
- Translate question, answer, document titles, and context sentences.
- Use batch processing, translation cache, and checkpointing.

Colab-specific commands, Google Drive paths, and long output logs are removed.
"""
...
def translate_sample(sample: Dict, tokenizer, model, batch_size: int = BATCH_SIZE) -> Dict:
    """Translate one HotpotQA sample into Indonesian."""
    translated = {
        "_id": sample.get("_id"),
    ...
    for original_title, _ in sample["context"]:
        if original_title in sample.get("supporting_titles", []):
            for source_doc, translated_doc in zip(sample["context"], translated_context):
                source_title = source_doc[0]
                translated_title = translated_doc[0]

                if source_title == original_title:
                    supporting_titles.append(translated_title)
                    break

    translated["supporting_titles"] = supporting_titles
...
def translate_file(
    ...
    """Translate one HotpotQA JSON file."""
...
def main() -> None:
    """Run HotpotQA translation."""
    parser = argparse.ArgumentParser(description="Translate HotpotQA data to Indonesian.")
    parser.add_argument("--train-input", required=True, help="Path to HotpotQA train JSON.")
    parser.add_argument("--train-output", required=True, help="Path to translated train JSON.")
    parser.add_argument("--val-input", required=True, help="Path to HotpotQA validation JSON.")
    parser.add_argument("--val-output", required=True, help="Path to translated validation JSON.")
    parser.add_argument(
        "--checkpoint-dir",
        default="checkpoints/hotpot_translation",
    ...
    translate_file(
        input_file=args.val_input,
        output_file=args.val_output,
        checkpoint_file=os.path.join(args.checkpoint_dir, "val_checkpoint.json"),
        tokenizer=tokenizer,
        model=model,
    )
